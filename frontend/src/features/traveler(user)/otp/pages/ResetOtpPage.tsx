import React, { useEffect } from "react";
import { useNavigate } from "react-router-dom";
import ResetOtpForm from "../components/ResetOtpForm";

interface PendingPasswordReset {
  userId: string;
  email: string;
  expiresAt: number;
}

const ResetOtpPage: React.FC = () => {
  const navigate = useNavigate();

  const pendingPasswordReset = sessionStorage.getItem("pendingPasswordReset");

  useEffect(() => {
    if (!pendingPasswordReset) {
      navigate("/forgot-password", { replace: true });
    }
  }, [navigate, pendingPasswordReset]);

  if (!pendingPasswordReset) {
    return null;
  }

  const resetData: PendingPasswordReset = JSON.parse(pendingPasswordReset);

  return (
    <div className="min-h-screen bg-[#f4faff] flex flex-col items-center justify-center p-6 font-sans">
      <div className="flex flex-col items-center gap-2 mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
        <h1 className="text-3xl font-black text-[#6c63ff] tracking-tighter">
          TripNest
        </h1>

        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em]">
          Verify Your Identity
        </p>
      </div>

      <ResetOtpForm userId={resetData.userId} email={resetData.email} />
    </div>
  );
};

export default ResetOtpPage;
