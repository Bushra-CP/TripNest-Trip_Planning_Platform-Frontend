...
import { googleAuthThunk } from "../redux/authThunk";
    ...
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const data = await dispatch(
          googleAuthThunk({ googleAcessToken: tokenResponse.access_token }),
        ).unwrap();
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => googleAuth()}
        className="w-full h-14 border rounded-2xl border-[#cfdce4] flex justify-center items-center gap-3 hover:bg-gray-50"
      >
        {/* SVG */}
